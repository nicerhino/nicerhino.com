...
const callsToAction = [
  { name: "Request callback", href: "#", icon: PlayCircleIcon },
            ...
            >
              <div className="p-4">
                {services.map((item) => (
                  ...
                  >
                    <div className="mt-1 flex size-11 flex-none items-center justify-center rounded-lg bg-slate-700 group-hover:bg-slate-500">
                      <item.icon
                        aria-hidden="true"
      ...
      <Dialog open={mobileMenuOpen} onClose={setMobileMenuOpen} className="lg:hidden">
        ...
        <DialogPanel className="fixed inset-y-0 right-0 z-10 flex w-full flex-col justify-between overflow-y-auto bg-white sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
            ...
            <div className="mt-6 flow-root">
              <div className="-my-6 divide-y divide-gray-500/10">
                <div className="space-y-2 py-6">
                  {services.map((item) => (
                    <a
                      key={item.name}
                      href={item.href}
                      className="group -mx-3 flex items-center gap-x-6 rounded-lg p-3 text-base/7 text-slate-200 hover:bg-gray-50"
                    >
                      <div className="flex size-11 flex-none items-center justify-center rounded-lg bg-slate-400 group-hover:bg-slate-500">
                        <item.icon
                          aria-hidden="true"
                          className="size-6 group-hover:text-indigo-600"
                        />
                ...
                <div className="space-y-2 py-6">
                  <a
                    href="#"
                    className="-mx-3 block rounded-lg px-3 py-2 text-base/7 text-slate-200 hover:bg-gray-50"
                  >
                    Features
                  </a>
                  <a
          ...
          <div className="sticky bottom-0 grid grid-cols-2 divide-x divide-gray-900/5 bg-gray-50 text-center">
            {callsToAction.map((item) => (
              <a key={item.name} href={item.href} className="p-3 text-base/7 hover:bg-gray-100">
                {item.name}
              </a>
            ))}
          </div>
